...
**Admin（管理员）**:
`role === "admin"` 的 User。拥有除"角色管理"外的全部后台权限：封禁/解封普通 User、删除/恢复页面、切换 Editor mode、管理讨论。**不能**任命或撤销管理员，也不能封禁/降级 站长——那是 站长 的专属权。由 站长 在后台任命产生。
_Avoid_: Superuser

**站长（Owner）**:
全站**恰好一个**的最高权限持有者，是一名被指定的 Admin——`role` 仍为 `"admin"`，**不是**独立的 role 取值，而是 Admin 的超集。唯一能变更他人 role（任命/撤销 Admin）的身份。作为信任根，站长**不可被任何 Admin 封禁或降级**，只能通过直接改数据库来变更——这是它与普通 Admin 的根本区别。
_Avoid_: Superuser、超管（口语可，规范词用 站长 / Owner）；不要把 站长 当成第三个 role 字符串
...
某个 User 对一个 Page ID 的一次连续编辑上下文，维持服务器已确认状态、至多一个结果未知的提交及其后的 Local draft 之间的因果顺序。刷新可以延续同一会话；另一个标签页是独立会话。
...
- **"编辑者" ≠ 角色**：当有人说"编辑者能不能删页面"时，先确认指的是"处于编辑模式下的 User"，而非某个独立角色。本项目有 User / Admin / 站长 三级，其中 站长 是 Admin 的超集（同一个 `role: "admin"`，是被另行指定的那一个 Admin），并非第三个 role 字符串。
...
> **甲**：那 Admin 怎么来的？
> **乙**：由 站长在后台任命。站长是全站唯一的最高权限者，作为信任根只能改库产生；普通 Admin 之间无权互相任免，也动不了站长。
