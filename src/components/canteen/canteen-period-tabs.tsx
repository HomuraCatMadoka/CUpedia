    ...
    >
      {periods.map((period) => (
        <button
          ...
          className={cn(
            "min-h-11 flex-1 touch-manipulation border-b-2 px-3 text-base font-medium sm:px-4 sm:text-sm",
            value === period
    ...
    <div role="tablist" aria-label="视图" className="flex flex-wrap gap-x-4 gap-y-2">
      {modes.map((mode) => (
        <button
          ...
          className={cn(
            "min-h-11 touch-manipulation text-base font-medium underline-offset-4 sm:text-sm",
            value === mode
