...
function seedState(): MockState {
  ...
  const rankingCanteens: Canteen[] = Array.from({ length: 17 }, (_, index) => ({
    id: `mock-canteen-ranking-${String(index + 4).padStart(2, "0")}`,
    name: `演示食堂 ${String(index + 4).padStart(2, "0")}`,
    location: `演示区域 ${String(index + 4).padStart(2, "0")}`,
    announcement: null,
    createdAt: t,
    updatedAt: t,
  }));
