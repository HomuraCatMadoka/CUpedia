...
const PIN_TYPE_PATHS: Record<CampusMapPinType, string> = {
  toilet:
...
export function pinTypeMarkerIcon(pinType: CampusMapPinType) {
  return icon(PIN_TYPE_PATHS[pinType], `data-pin-type-icon="${pinType}"`);
}
...
export function pinTypeMarkerContent(input: {
  markerKey: string;
  name: string;
  buildingName: string;
  floorLabel: string;
  ...
  return markerButton({
    ...
    color: input.color,
    icon: pinTypeMarkerIcon(input.pinType),
  });
}
