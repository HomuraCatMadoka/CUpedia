...
describe("parseMenuItemsJson", () => {
  it("parses a JSON array of menu items", () => {
    const rows = parseMenuItemsJson(`[
      { "name": "宫保鸡丁", "price": 28, "mealPeriods": ["lunch"] },
      { "name": "皮蛋瘦肉粥", "price": 12, "mealPeriods": ["breakfast"], "sortOrder": 1 }
    ]`);
    ...
    expect(rows[1].mealPeriods).toEqual(["breakfast"]);
  });

  it("accepts wrapped { items: [...] } shape", () => {
    const rows = parseMenuItemsJson({
      items: [{ name: "演示菜品", price: 10, mealPeriods: ["dinner"] }],
    });
