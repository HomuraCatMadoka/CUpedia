...
**餐段 Tab**: 食堂详情页按该店实际供应的餐段展示早/午/晚；仅一种餐段（如只做午餐、甜品全天）时不显示餐段 Tab。默认 Tab 在**客户端**按 `Asia/Hong_Kong` 计算并夹到可用餐段。规则见 PRD / `canteen-meal-period.ts`（11:30→午，17:30→晚；仅当同时有午/晚时，14:30–17:29 显示午后提示）。
